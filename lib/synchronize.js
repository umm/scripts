...
const synchronize = (source, destination, patterns, overwrite_options, callback) => {
  ...
  // 対象ファイル情報一覧を構築
  const source_file_list = patterns
  // パターンをファイル・ディレクトリの一覧に展開
  .flatMap(
    (pattern, index) => glob
    .sync(path.join(source, pattern.pattern))
    .map((entry) => {
      let file_path = entry.replace(new RegExp(`^${source}/`), '');
  ...
  .flatMap(
    (pattern, index) => {
      let list = [];
      if (pattern.extract_directory && fs.statSync(pattern.source_path).isDirectory()) {
        list = glob
  ...
  // 対象ファイル情報一覧を構築
  const destination_file_list = patterns
  // パターンをファイル・ディレクトリの一覧に展開
  .flatMap(
    (pattern, index) => glob
    .sync(path.join(destination, pattern.pattern))
    .map((entry) => {
      let file_path = entry.replace(new RegExp(`^${destination}/`), '');
